let btnNex = document.querySelectorAll('.film-next');
let btnPre = document.querySelectorAll('.film-prevous');
let parentList = document.querySelector('.Films-list')
let click = 0;
for ( var i=0;i<btnNex.length ; i++) {
	btnNex[i].addEventListener('click',nextFilm)
	function nextFilm (e) {
		let btnNext = e.target.previousElementSibling;
		click++;
		btnNext.style.transform= "translateX(-135rem)";
		btnNext.style.transition= "all 2s ease";
		if (click >= 2){
			click = 1;
		}
		console.log(click);
		let btnPreCurrent = btnNext.previousElementSibling.previousElementSibling;
		console.log(btnPreCurrent);
		if(click == 1) {
			e.target.classList.add('disabled')
			btnPreCurrent.classList.remove('disabled')
		}
		if(click == 0) {
			e.target.classList.remove('disabled')
			btnPreCurrent.classList.add('disabled')
		}
	}
}
for ( let i=0;i<btnPre.length ; i++) {
	btnPre[i].addEventListener('click',preFilm)
	function preFilm (e) {
